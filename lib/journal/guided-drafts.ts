...
/** Entwurfstext für Markdown-Inhalt (wie Figma-Simulation). */
export function generateGuidedDraftMarkdown(title: string): string {
  if (!title.trim()) {
    return "Schreiben Sie hier Ihre Erklärung für Patienten.\n\n\nDenken Sie daran: kurz, klar und verständlich.";
  }
  return (
    GUIDED_BODIES[title] ??
    "Schreiben Sie hier Ihre Erklärung für Patienten.\n\n\nDenken Sie daran: kurz, klar und verständlich."
  );
}
