...
export function TrackerInboxPanel({ items }: TrackerInboxPanelProps) {
  ...
  return (
    <div className="yd-tracker-v4-inbox yd-tracker-v8-inbox yd-tracker-v9-inbox yd-clinical-control flex h-full min-h-0 flex-col">
      ...
      <ul
        className="yd-tracker-v4-inbox__list yd-tracker-v8-inbox__list yd-tracker-v9-inbox__list min-h-0 flex-1 overflow-y-auto overscroll-y-contain [-webkit-overflow-scrolling:touch]"
            ...
            return (
              <li key={item.id}>
                ...
                >
                  <button
                    type="button"
                    className="yd-tracker-v9-inbox-card__body"
                    onClick={() => goToCase(item.id)}
                    aria-current={isActive ? "page" : undefined}
                  >
                    <div className="yd-tracker-v9-inbox-card__text">
                      <span className="yd-tracker-v9-inbox-card__name" title={patientName}>
                        {patientName}
                      </span>
                      <span className="yd-tracker-v9-inbox-card__concern" title={concern}>
                        {concern}
                      </span>
                      <span className="yd-tracker-v9-inbox-card__time">{timeLabel}</span>
                    </div>
                  </button>
