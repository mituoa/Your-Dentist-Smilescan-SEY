...
const PILL_QUIET =
  "border-[rgba(180,198,218,0.38)] bg-white/90 text-[#1a3348] hover:border-[rgba(26,79,156,0.28)] hover:bg-[#f8fbfe] hover:shadow-[0_2px_10px_rgba(26,79,156,0.08)]";

const PILL_PRIMARY =
  "border-[rgba(22,61,122,0.22)] bg-[#1A4F9C] text-white shadow-[0_4px_14px_rgba(22,61,122,0.22)] hover:brightness-[1.03] active:scale-[0.99]";
  ...
  return (
    ...
    >
      ...
      <Link href="/relay?panel=messages" className={cn(PILL_BASE, PILL_QUIET)}>
        <MessageSquarePlus className="h-4 w-4 shrink-0 text-[#1A4F9C]" strokeWidth={1.75} />
        Relay Nachricht
      </Link>
      ...
      <Link href="/inbox" className={cn(PILL_BASE, PILL_QUIET, "max-lg:hidden")}>
        <ClipboardCheck className="h-4 w-4 shrink-0 text-[#1A4F9C]" strokeWidth={1.75} />
        Antwort vorbereiten
      </Link>
      ...
      >
        <UserPlus className="h-4 w-4 shrink-0 text-[#1A4F9C]" strokeWidth={1.75} />
        Tracker
      </Link>
    </nav>
  );
